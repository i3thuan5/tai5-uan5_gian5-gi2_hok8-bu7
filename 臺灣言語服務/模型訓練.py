...
class 模型訓練(程式腳本):
    ...
    def 訓練摩西翻譯模型(cls, 語料資料夾, 模型資料夾, 語言):
        ...
        if cls._語言判斷.是漢語(語言):
            ...
            平行華語, 平行母語, 母語文本 = cls._一般語料訓練(語言資料夾)
    ...
    def _漢語語料訓練(cls, 語言資料夾, 翻譯模型資料夾):
        ...
        母語語言模型 = cls._文本檔轉模型物件(語言資料夾, 補漢字音標的母語文本)
        return (
    ...
    def _檔案正規化(cls, 母語辭典, 母語語言模型, 檔名):
        for 一逝 in cls._讀檔案(檔名):
            句物件 = 拆文分析器.分詞句物件(一逝)
            揣詞句物件 = 拄好長度辭典揣詞.揣詞(母語辭典, 句物件)
            選好句物件 = 語言模型揀集內組.揀(母語語言模型, 揣詞句物件)
            yield 物件譀鏡.看分詞(選好句物件)
