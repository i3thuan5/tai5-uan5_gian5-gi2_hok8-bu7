...
#                     全部詞.add(一項)
    ...
    def _音檔資訊(cls, 影音所在, 音檔名, 音檔目錄, 音檔對應頻道):
        print(
            ...
            "sox -G --ignore-length -t wav - -b 16 -c 1 -r 16k -t wav - | "
            .format(影音所在),
            file=音檔目錄
        )
#             sw02001-A sw02001 A
        ...
        if 0.0 <= 開始 and 開始 + 0.1 < 結束 and 結束 <= 音檔長度:
            if 原本語者 in ['', '無註明']:
                ...
                try:
                    ...
                    語者 = '{0}-{1:07}{2}'.format(來源, 第幾个人, 語者名)
                    語者名對應輸出名[來源, 語者名] = 語者
                    第幾个人 += 1
            語句名 = '{0}-{1}-ku{2:07}'.format(語者, 音檔名.split('-')[-1], 第幾句)
